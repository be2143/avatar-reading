        ...
        setActivities((prevActivities) => {
            return prevActivities.map((activity) => {
                if (activity.title === item.title) {
                    return { ...activity, planned: true, state: "not done" };
                }
                return activity;
            });
        });
    ...
    const handleSave = async () => {
        ...
        try {
            const response = await fetch('/api/activities', {
                ...
                body: JSON.stringify({ activities: plannedActivities }),
            });
        ...
        <DndProvider backend={HTML5Backend}>
            <div className="p-6 bg-[url('/bg.png')] min-h-screen">
                <div className="mt-5 ml-20">
                    ...
                    {loading ? (
                        <div className="text-gray-500 text-lg">Loading activities...</div>
                    ) : (
                        <>
                                ...
                                {activities
                                    ...
                                    .map((activity, index) => (
                                        <ActivityCard
                                            ...
                                            onDrop={() => { }}
                                            onDelete={() => handleDelete(activity.title)}
                                            isDraggable={true}
                                        />
                                    ))}
