...
async function addStoriesToDB() {
    try {
        ...
        // Transform storiesToAdd to map 'id' to 'dummyStoryId' and remove 'id'
        const transformedStories = storiesToAdd.map(story => {
            const newStory = { ...story, dummyStoryId: story.id.toString() };
            delete newStory.id; // Remove the original 'id' field
            return newStory;
        });

        const result = await Story.insertMany(transformedStories);
        console.log(`Successfully added ${result.length} stories to the database.`);
